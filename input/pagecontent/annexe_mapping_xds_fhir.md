...
| `authorInstitution` | `source.extension:authorOrg` → `Reference(AS Organization Profile)` `{c}` [0..1] | ❌ pas de correspondance possible en l'état pour les structures sans fiche dans l'Annuaire Santé — contrairement à `author`/`source` eux-mêmes, cette extension est restreinte **exclusivement** à `AS Organization Profile`, sans cible générique (`Organization` simple) en alternative. Une structure identifiée seulement par un identifiant interne, ou un éditeur de SNR identifié par son SIREN, n'a donc aucune cible conforme à utiliser. Décontrainte de PDSm nécessaire (ajout d'une cible `Organization` générique, à l'image de ce qui existe déjà pour `author`/`source`) |
