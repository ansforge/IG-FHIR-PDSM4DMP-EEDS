...
- **XDS** ([`Update Document Set [ITI-57]`](https://esante.gouv.fr/sites/default/files/media_entity/documents/ci-sis_service_volet-partage-documents-sante_v1.16.4.pdf#page=27), §3.3.5.1.1 du volet — mise à jour de `confidentialityCode`, cas du masquage/démasquage [TD3.3a](transaction_td3.3a.html)/[TD3.3b](transaction_td3.3b.html)) soumet à chaque mise à jour une **nouvelle fiche**, avec un nouvel `entryUUID`, mais en conservant le même `uniqueId` et le même `logicalID` ; `version` est incrémenté (exemple chiffré : fiche A2 `entryUUID=22-22-22-22-22` → fiche A2-2 `entryUUID=45-456-456-456-45`, même `uniqueId`/`logicalID`, `version` 1→2, cf. [Figure 21 du volet](https://esante.gouv.fr/sites/default/files/media_entity/documents/ci-sis_service_volet-partage-documents-sante_v1.16.4.pdf#page=29)). À la différence, une mise à jour d'`availabilityStatus` (archivage/dépublication, [TD3.3c](transaction_td3.3c.html)/[TD3.3d](transaction_td3.3d.html), [§3.3.5.1.2](https://esante.gouv.fr/sites/default/files/media_entity/documents/ci-sis_service_volet-partage-documents-sante_v1.16.4.pdf#page=31)) ne crée **aucune nouvelle fiche** : seule la métadonnée est modifiée en place sur la fiche existante, sans changement d'`entryUUID`.
- **PDSm** modélise la mise à jour de `confidentialityCode` par un **PATCH sur la ressource `DocumentReference` existante** ([TD3.3a](transaction_td3.3a.html)/[TD3.3b](transaction_td3.3b.html)) : `identifier` (slice `entryUUID`) reste donc inchangé d'une mise à jour à l'autre, là où XDS lui attribue une nouvelle valeur à chaque fois.
