...
|---|---|---|---|---|
| Alimentation — nouveau document | TD2.1 / TD2.2 (DMP_2.1a/2.2a) | ITI-41 Provide and Register Document Set-b | ITI-65 (Flux 01) **ou** ITI-105 (Flux 09) | ITI-65 : Bundle `transaction` / ITI-105 : POST d'une seule `DocumentReference`<br>Ex. : `POST [base]/DocumentReference` (ITI-105) ou `POST [base]` (ITI-65, Bundle `transaction`) |
| Alimentation — remplacement | TD2.1 / TD2.2 (DMP_2.1b/2.2b) | ITI-41 + association RPLC | ITI-65 (Flux 01) **ou** ITI-105 (Flux 09) | `relatesTo.code = replaces` (cardinalité `1..1` au remplacement). ITI-65 : ancienne fiche passée à `superseded` incluse dans le Bundle. ITI-105 : POST de la nouvelle fiche (`status = current`) avec `relatesTo[replaces]` ; le gestionnaire déprécie la cible<br>Ex. : `POST [base]` (Bundle `transaction` : nouveau `DocumentReference` + PATCH sur l'ancien) |
| Recherche de documents | TD3.1 (DMP_3.1) | ITI-18 Registry Stored Query (`FindDocuments`, `GetDocuments`) | ITI-67 Find Document References | Recherche REST FHIR (`search`) sur `DocumentReference` (HTTP GET/POST)<br>Ex. : `GET [base]/DocumentReference?patient.identifier=[ins]&status=current` |
| Recherche de lots / classeurs | TD3.1 (`FindSubmissionSets`) | ITI-18 Registry Stored Query | ITI-66 Find Document Lists | Recherche REST FHIR sur `List` (SubmissionSet / Folder)<br>Ex. : `GET [base]/List?patient.identifier=[ins]` |
| Consultation / récupération | TD3.2 (DMP_3.2) | ITI-43 Retrieve Document Set | ITI-68 Retrieve Document | Récupération du `Binary` via `attachment.url`<br>Ex. : `GET [DocumentReference.content.attachment.url]` |
| Masquer / démasquer aux professionnels | TD3.3a | ITI-57 Update DocumentEntry Metadata | Flux 03 — mise à jour des métadonnées (PATCH) | HTTP PATCH (conditional, sur identifiant métier) de `DocumentReference.securityLabel` 🐉<br>Ex. : `PATCH [base]/DocumentReference?identifier=[uniqueId]` (`securityLabel` → `MASQUE_PS`) |
| Visibilité patient / représentants légaux | TD3.3b | ITI-57 Update DocumentEntry Metadata | Flux 03 — mise à jour des métadonnées (PATCH) | HTTP PATCH de `DocumentReference.securityLabel` 🐉<br>Ex. : `PATCH [base]/DocumentReference?identifier=[uniqueId]` (`securityLabel` → `INVISIBLE_PATIENT`) |
| Archiver / désarchiver | TD3.3d | ITI-57 — `availabilityStatus` = `urn:asip:ci-sis:2010:StatusType:Archived` | Flux 03 — mise à jour des métadonnées (PATCH) | HTTP PATCH de l'extension `PDSm_isArchived` (booléen) sur `DocumentReference` / `List`<br>Ex. : `PATCH [base]/DocumentReference?identifier=[uniqueId]` (extension `isArchived` → `true`) |
| Supprimer un document (suppression logique) | TD3.3c | ITI-57 — `availabilityStatus` = `urn:asip:ci-sis:2010:StatusType:Deleted` | Flux 03 — mise à jour des métadonnées (PATCH) | HTTP PATCH de `DocumentReference.status` → `superseded` (dépublication). Pas de suppression physique (`DELETE` absent dans MHD). RMD (ITI-62/86) **non utilisé** par le DMP 🐉<br>Ex. : `PATCH [base]/DocumentReference?identifier=[uniqueId]` (`status` → `entered-in-error`) |

🐉 *Point encore ouvert sur cette transaction — voir la note dragon en bas de la page [TD3.3a](transaction_td3.3a.html), [TD3.3b](transaction_td3.3b.html) ou [TD3.3c](transaction_td3.3c.html) dédiée pour le détail.*
