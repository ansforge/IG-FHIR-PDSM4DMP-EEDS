...
| `authorPerson` | `author`→`.practitioner`d'un`PractitionerRole`**contained**, ou directement un`Practitioner`/`PractitionerRole`**contained**référencé par`author` | ✅ — le`Practitioner`visé ne peut être qu'une ressource**sœur**contenue (jamais imbriquée dans un autre`contained`, cf.[dom-2](https://hl7.org/fhir/R4/references.html#contained):**« Contained resources SHALL NOT contain additional contained resources »**), mais le binding`author`accepte, en plus du profil national`AS Practitioner Profile`, un`Practitioner`/`PractitionerRole`**générique non profilé**— celui-ci n'est soumis à aucune contrainte d'appartenance à l'Annuaire Santé (pas d'identifiant RPPS/FINESS exigé, pas de conformité à un profil externe), seules les règles de base FHIR (cardinalité, type) s'appliquant : une simple copie locale (nom + identifiant, exactement la donnée déjà portée par le type XCN de`authorPerson`) suffit à le satisfaire.`AS Practitioner Profile`reste pertinent pour un professionnel à identifiant national (résolvable dans l'Annuaire Santé) ; la variante générique couvre les autres catégories (identifiant interne, patient, système, SNR) |
| `authorInstitution` | `author`→`PractitionerRole.organization`, ou directement une`Organization`**contained**référencée par`author` | ✅ — même raisonnement que pour`authorPerson`: le binding`author`accepte une`Organization`générique non profilée en plus de`AS Organization Profile`, ce qui permet de couvrir par une simple copie locale (nom + identifiant, la donnée déjà portée par le type XON de`authorInstitution`) les structures sans fiche dans l'Annuaire Santé (éditeur de SNR identifié par SIREN, structure à identifiant interne) |
...
| `authorPerson` | `source`→`.practitioner`d'un`PractitionerRole`**contained**, ou directement un`Practitioner`/`PractitionerRole`**contained** | ✅ — même raisonnement que pour la métadonnée équivalente de la fiche (cf. §3.4.3, renvoi explicite en §3.5.3) : le binding`source`accepte, en plus de`AS Practitioner Profile`(professionnel à identifiant national, résolvable dans l'Annuaire Santé), un`Practitioner`/`PractitionerRole`générique non profilé pour les autres catégories (identifiant interne, patient, système, SNR), satisfaisable par une simple copie locale |
| `authorInstitution` | `source.extension:authorOrg`→`Reference(AS Organization Profile)`[0..1] | ⚠️ contrairement à`author`/`source`, je n'ai pas trouvé trace d'une cible générique (`Organization`non profilée) pour cette extension spécifique — sa définition affichée ne mentionne que`AS Organization Profile`, donc la réserve reste : résolvable seulement si la structure émettrice a une fiche dans l'Annuaire Santé (à vérifier auprès de la définition normative de l'extension IHE`ihe-authorOrg`telle que reprise par PDSm) |
