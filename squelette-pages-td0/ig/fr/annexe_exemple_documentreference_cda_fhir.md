# Exemple DocumentReference CDA / FHIR - Utilisation de PDSm dans le contexte d'EEDS v0.1.0

## Exemple DocumentReference CDA / FHIR

### Objectif

Cette annexe illustre la capacité d'un même point d'accès `DocumentReference` (par exemple la transaction **[ITI-67 Find Document References](transaction_td3.1a.md)**) à exposer indifféremment :
...
Le format du document n'est pas porté par la transaction utilisée pour y accéder (cf. [TD3.1a](transaction_td3.1a.md), [TD3.2](transaction_td3.2.md)), mais uniquement par les métadonnées du `DocumentReference` correspondant :
...
Le `DocumentReference` [example-annexe-docref-cda](DocumentReference-example-annexe-docref-cda.md) référence, via `content.attachment.url`, la ressource [Binary/example-annexe-binary-cda](Binary-example-annexe-binary-cda.md) qui contient le document CDA encodé en base64 (`Binary.data`).
...
Le `DocumentReference` [example-annexe-docref-fhir](DocumentReference-example-annexe-docref-fhir.md) référence, via `content.attachment.url`, la ressource [Bundle/example-annexe-fhir-document](Bundle-example-annexe-fhir-document.md), un `Bundle` de type `document` contenant une `Composition` et les ressources associées (`Patient`, `Practitioner`).
...
Le `Bundle` [example-annexe-searchset-documentreferences](Bundle-example-annexe-searchset-documentreferences.md), de type `searchset`, illustre la réponse que retournerait le système DMP à une recherche `DocumentReference` (TD3.1a) : les deux `DocumentReference` des exemples précédents y figurent côte à côte, chacun avec son propre format de contenu.

| | | |
| :--- | :--- | :--- |
| [example-annexe-docref-cda](DocumentReference-example-annexe-docref-cda.md) | CDA (`application/xml`) | `Binary/example-annexe-binary-cda` |
| [example-annexe-docref-fhir](DocumentReference-example-annexe-docref-fhir.md) | FHIR (`application/fhir+json`) | `Bundle/example-annexe-fhir-document` |
